...
double ulim[9] = {320,320,170,170,110,40,40,500000000, 500000000};
...
KUKA_INVKIN::KUKA_INVKIN() {
	...
	if (!init_robot_model()) exit(1); 
        ...
        _my_pub_red = _nh.advertise< std_msgs::Float64 > ("/q_red_cmd",0);
...
VectorXd KUKA_INVKIN::estimated_dynamic(VectorXd q_in){
	...
	size_t num_of_steps = integrate_adaptive(make_dense_output< rosenbrock4< double > >(1.0e-3, 1.0e-6), //before was e-4 e-7
...
void KUKA_INVKIN::ctrl_loop() {
        ...
        ref_qr << 0.1248, -0.0295, 0.1240, 1.5410, -1.5746, -1.8195,-1.5713; // inverse kinematics solved
        //ref_qr << -0.1252, -0.0295, -0.1236, 1.5410, -1.5670, -1.3221,-1.5713; // inverse kinematics solved
        ref_qc << 0.1,1.18;
        ...
        msg_r = ros::topic::waitForMessage<sensor_msgs>("/iiwa_q"); //to know the robot status
        ...
        if(cmd_crane.data[1] > 0 && cmd_crane.data[1] <= 2) my_k = 10;
        if(cmd_crane.data[1] > 2 && cmd_crane.data[1] <= 5 ) my_k = 20;
        if(cmd_crane.data[1] > 5 && cmd_crane.data[1] <= 6 ) my_k = 30;
        if(cmd_crane.data[1] > 6 && cmd_crane.data[1] <= 7 ) my_k = 40;
        if(cmd_crane.data[1] > 8 && cmd_crane.data[1] <= 9 ) my_k = 100;
        if(cmd_crane.data[1] > 9 && cmd_crane.data[1] <= 9.5) my_k = 150;
        if(cmd_crane.data[1] > 9.5) my_k = 200;
        ...
        //file_ERG<<time_ro<<" "<<time_delta<<" "<<time_ERG<<" ";
        //file_ERG<<"\n";
        ...
        cmd_crane.data[1] = (-qv(7)+0.2)*100;
        
        //cout<<"cmd: "<<qc_des(0)<<" "<<x_off<<endl;
        cout<<"des_cmd: "<<cmd_crane.data[1]<<endl;
         _my_pub2.publish(cmd_crane);
        //cin>>stop_flag;
        //cmd_pub_red.data = qv(0) +  0.7854;
        //_my_pub_red.publish(cmd_pub_red);
...
int main(int argc, char** argv) {
        ...
        cout<<"START_X"<<endl;
	ros::init(argc, argv, "my_script_cpp_x");
	KUKA_INVKIN ik;
	ik.run();
