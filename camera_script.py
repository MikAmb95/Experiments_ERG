...
def talker():
    ...
    pub_q = rospy.Publisher('block_state', Float64MultiArray, queue_size=1)
    ...
    k=0
    q_curr=0 
    q_prev=np.array([float(x_inp)*0.001,0,0,float(l_inp)*0.001,0,0,0])  
    first_time= True 
    ...
    while not rospy.is_shutdown():
        ...
        #print("l_inp", l_inp)
        l = float(l_inp)*0.001
        x_off = float(x_inp)*0.001
        #print("x_2", 0.5+float(x_inp))
        ...
        if len(corners)>0: # if there is at least one marker detected
            ...
            if markNum != 0:
                ...
                alpha= (np.pi)
                R_block2board= Roty(alpha)
                R_block2cam=np.dot(R_board2cam,R_block2board)

                alpha = (-np.pi/2)
                R_block2board= Rotx(alpha)
                R_block2cam=np.dot(R_block2cam,R_block2board)
                ...
                ##################CHANGE_IF_NEEDED##################
                x= 460 #2450(the distance structure,fix), 830
                y= -900 # last term is tuning 
                z= -2260 #fix from the top to the wood plate (-2235)
                ##################CHANGE_IF_NEEDED##################
                ...
                t1=  pt_wrt_crane[0]*0.001+0.006*0 #0.006 is an offset
                t2=  pt_wrt_crane[1]*0.001+0.008*0+0.2612*0+x_off
                t3=  pt_wrt_crane[2]*0.001+0.0094*0 #0.0094 is an offset
                #print(pt_wrt_crane)
                ...
                if q_curr[2]*(180/np.pi)>300:
                    q_curr[2] = q_curr[2]-2*np.pi

                q_curr[1] = q_curr[1]+3*(np.pi/180) *0
                q_curr[2] = q_curr[2]-4*(np.pi/180)*0
                q_curr[4] = q_curr[4]+np.pi/2 
                q_curr_ext = np.array([x_off,q_curr[0],q_curr[1],l,q_curr[2],q_curr[3],q_curr[4]])
                
                
                q_dot= (2*N*q_curr_ext - 2*N*q_prev- ((1/frq)*N-2)*q_dot)/(((1/frq)*N+2))# with filter
                q_prev = q_curr_ext
                print("q_curr_ext",q_curr_ext[1]*(180/np.pi)," ",q_curr_ext[2]*(180/np.pi)," ",q_curr_ext[4]*(180/np.pi)," ",q_curr_ext[5]*(180/np.pi)," ",q_curr_ext[6]*(180/np.pi))
                #print("q_dot",q_dot)
                #q_curr=np.random.normal(0,0.0001,5)
                #data.data= np.array([x_off,q_curr[0],q_curr[1],l,q_curr[2],q_curr[3],q_curr[4]])
                data.data= np.array([x_off,q_curr[0],q_curr[1],l,q_curr[2],q_curr[3],q_curr[4],q_dot[0],q_dot[1],q_dot[2],q_dot[3],q_dot[4],q_dot[5],q_dot[6]])
                #data.data= np.array([t1,t2,t3,r11,r12,r13,r21,r22,r23,r31,r32,r33])
                #print("x_off",x_off," l",l)
                my_time = time.time()-start
                my_time2 = time.time()-start2
                #print("Time",my_time2)
                #if my_time<15:
                #    index = index + 1
                #    my_file.write("vec_data("+str(index)+",:)=["+str(x_off)+" "+str(q_curr[0])+" "+str(q_curr[1])+" "+str(l)+" "+str(q_curr[2])+" "+str(q_curr[3])+" "+str(q_curr[4])+" "+str(q_dot[0])+" "+str(q_dot[1])+" "+str(q_dot[2])+" "+str(q_dot[3])+" "+str(q_dot[4])+" "+str(q_dot[5])+" "+str(q_dot[6])+" "+str(my_time)+"];\n")
                #else:
                #    my_file.close()
